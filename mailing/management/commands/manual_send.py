...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        print(f'min_datatime: {min_datatime}')
        max_datatime = current_datatime + timedelta(minutes=30)
        print(f'max_datatime: {max_datatime}')
        print()
        for item in mailing_list:
            # надо получить datatime отправки рассылки, зная data_start, time, period
            print(f'данные из mailinga: {item.time, item.period, item.period_id, item.data_start}')
            if item.period_id == 1 and item.data_start <= current_date:     #если рассылка ежедневная и дата старта настала
                ...
                print(f'полученный mailing_datatime ежедневной рассылки: {mailing_datatime}')
                if min_datatime < mailing_datatime <= max_datatime:
                    ...
                    print('время отправки рассылки вне текущего интервала')
                    print()
                ...
                print(f'полученный mailing_datatime ежемесячной рассылки: {mailing_datatime}')
                if min_datatime < mailing_datatime <= max_datatime:
                    spammer_email = item.user_email
                    client_list = Client.objects.filter(user_email=spammer_email)
                    print(f'client_list', client_list)  # для отладки
                    for client in client_list:
                        try:
                            send_mail(subject=item.subject, message=item.mailing_text, from_email=spammer_email, recipient_list=[client.client_email, ])
                        except (smtplib.SMTPRecipientsRefused, smtplib.SMTPDataError) as smtp_error:
                            print(smtp_error)
                else:
                    print('время отправки рассылки вне текущего интервала')
